...
const form = useForm({
    title: props.licitacion.title || '',
    company_id: props.licitacion.company_id ?? props.companies[0]?.id ?? null,
    company_letterhead_id: props.licitacion.company_letterhead_id ?? null,
    process_type: props.licitacion.process_type || props.processTypes[0] || 'publica',
    legal_signer_name: props.licitacion.legal_signer_name || '',
    regulation_ids: props.licitacion.regulation_ids || [],
    bases_document: null,
});
...
const submit = () => {
    form.put(route('licitacion.update', props.licitacion.id));
};
</script>

<template>
    <div>
        <Head :title="`Editar · ${licitacion.title}`" />

        <AuthenticatedLayout>
            <template #header>
                <h2 class="text-xl font-semibold leading-tight text-gray-800">Editar Expediente de Licitación</h2>
            </template>
                ...
                <div class="mx-auto max-w-4xl sm:px-6 lg:px-8">
                    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                        <form class="space-y-6" @submit.prevent="submit">
                            ...
                            <div>
                                <InputLabel value="Reemplazar bases de licitación (PDF)" />
                                <p class="mb-2 text-xs text-gray-500">Actual: {{ licitacion.bases_document_original_name || 'Sin archivo' }}</p>
                                <input
                                    type="file"
