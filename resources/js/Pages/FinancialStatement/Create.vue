...
const submit = () => {
    form.post(route('estado-financiero.store', props.company.id));
};
</script>

<template>
    <div>
        <Head title="Nuevo Estado Financiero" />

        <AuthenticatedLayout>
            <template #header>
                <h2 class="text-xl font-semibold leading-tight text-gray-800">Nuevo Estado Financiero · {{ company.nombre }}</h2>
            </template>
