...
const props = defineProps({
    ...
    taxDeclaration: { type: Object, required: true },
});

const form = useForm({
    periodicity: props.taxDeclaration.periodicity,
    year: props.taxDeclaration.year,
    month: props.taxDeclaration.month,
    document: null,
});

const submit = () => {
    form.put(route('declaracion.update', [props.company.id, props.taxDeclaration.id]));
};
</script>

<template>
    <div>
        <Head title="Editar Declaración" />

        <AuthenticatedLayout>
            <template #header>
                <h2 class="text-xl font-semibold leading-tight text-gray-800">Editar Declaración de Impuestos · {{ company.nombre }}</h2>
            </template>
                ...
                <div class="mx-auto max-w-3xl sm:px-6 lg:px-8">
                    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                        <form class="space-y-6" @submit.prevent="submit">
                            ...
                            <div>
                                <InputLabel value="Reemplazar archivo (opcional)" />
