...
const form = useForm({
    title: props.letterhead.title,
    city: props.letterhead.city || '',
    contact_name: props.letterhead.contact_name || '',
    contact_position: props.letterhead.contact_position || '',
    phone: props.letterhead.phone || '',
    email: props.letterhead.email || '',
    body_template: props.letterhead.body_template || '',
    is_default: !!props.letterhead.is_default,
});

const submit = () => {
    form.put(route('letterhead.update', [props.company.id, props.letterhead.id]));
};
</script>

<template>
    <div>
        <Head title="Editar Hoja Membretada" />

        <AuthenticatedLayout>
            <template #header>
                <h2 class="text-xl font-semibold leading-tight text-gray-800">Editar Hoja Membretada · {{ company.nombre }}</h2>
            </template>
                    ...
                    <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                        <form class="space-y-6" @submit.prevent="submit">
                            ...
                            <div>
                                ...
                                <textarea v-model="form.body_template" class="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm" rows="8" />
